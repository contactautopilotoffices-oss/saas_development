/**
 * UniversalQRScannerModal — QR Code and Barcode Scanner Modal
 *
 * A unified scanning modal supporting three input modes: live camera feed, image gallery
 * upload, and manual text entry. It detects and categorizes scanned data into one of
 * four result types (checklist, stock item, barcode, unknown) using pattern matching and
 * type detection heuristics.
 *
 * KEY PROPS:
 *   - onResult: (result: QRScanResult) => void — callback fired when a valid scan is detected
 *   - onClose:  () => void — called when the user dismisses the modal or scan is complete
 *   - title?:   string — optional modal title (defaults to 'Universal Scanner')
 *
 * QRScanResult union type:
 *   - { type: 'checklist', templateId: string } — matched /checklist/{id} URL pattern
 *   - { type: 'stock',    itemId: string, raw: string } — JSON with item_id field
 *   - { type: 'barcode',  value: string } — non-URL short string treated as a barcode
 *   - { type: 'unknown',  raw: string } — URL or unrecognized pattern
 *
 * KEY STATE:
 *   - scanMode:    ScanMode ('camera' | 'gallery' | 'manual') — active input mode
 *   - cameraActive: boolean — whether the html5-qrcode scanner is currently running
 *   - loading:     boolean — true during camera startup
 *   - error:       string | null — last error message (e.g. camera denied)
 *   - detected:    { label: string } | null — success banner shown after successful scan
 *   - galleryProcessing: boolean — true while an uploaded image is being scanned
 *   - manualInput: string — current value of the manual text input
 *
 * INTEGRATION:
 *   - Dynamically imports html5-qrcode (lazy load) to avoid bloating the initial bundle
 *   - Uses the native BarcodeDetector API as a first-pass for gallery scans, falling back to html5-qrcode
 *   - html5-qrcode supports: QR_CODE, CODE_128, CODE_39, EAN_13, EAN_8, UPC_A, ITF, DATA_MATRIX
 *   - Gracefully handles camera transition errors with an automatic retry after a 400ms delay
 *
 * UI/UX PATTERNS:
 *   - Three tab-like mode buttons (Camera, Photo Upload, Manual ID) with icon + label
 *   - Camera feed always stays mounted in the DOM for lifecycle stability; shown/hidden via CSS
 *   - Success detection shows a green banner (CheckCircle2) before closing
 *   - Error shows a rose-colored alert banner
 *   - Gallery uses a multi-pass scan strategy: raw file, normal-processed, high-contrast, upscaled
 *   - Closes via backdrop click or X button; properly stops and clears the camera on unmount
 *   - Uses refs for scanner instance, mounted state, result flag, and mode-transition guard to avoid stale closures
 */
'use client';

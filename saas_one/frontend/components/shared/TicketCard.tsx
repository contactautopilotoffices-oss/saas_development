/**
 * TicketCard — Standard Ticket Card Component
 *
 * THE canonical ticket card used throughout the application. Every ticket rendered in
 * a list, grid, or flow map MUST use this component. Its contract is documented in
 * TICKET_CARD_CONTRACT.md.
 *
 * KEY PROPS (all required unless marked optional):
 *   - id / title / priority / status:  Core ticket identity and state
 *   - ticketNumber / createdAt:         Metadata for display and sorting
 *   - assignedTo / assigneePhotoUrl:    Current assignee (shown when status is not OPEN)
 *   - photoUrl:                         Optional site photo thumbnail
 *   - propertyName:                     Shown in Super Admin view to identify the property
 *   - escalationChain:                  Ordered array of MSTs in the escalation path (last = current)
 *   - raisedByTenant:                    Adds amber border highlight when ticket is tenant-raised
 *   - hasMaterialRequest:                Shows a "Materials" badge for procurement-linked tickets
 *   - onClick:                           Primary action: open the ticket detail
 *   - onEdit / onDelete / onValidate / onReject: Optional per-ticket action callbacks
 *
 * KEY STATE:
 *   - elapsedSec: number — live counter of seconds since ticket creation, updated every second
 *   - shareOpen: boolean — controls the ShareModal visibility
 *
 * INTEGRATION:
 *   - Renders ShareModal as a sub-component for ticket sharing
 *   - Uses TicketCardProps interface exported alongside the component for consuming components
 *   - Priority/status styling uses static maps (PRIORITY_STYLES / STATUS_STYLES)
 *
 * UI/UX PATTERNS:
 *   - Container-query responsive sizing using CSS @container and clamp() for fluid typography/layout
 *   - Critical priority tickets pulse with a red glow animation (framer-motion keyframe loop)
 *   - Live elapsed timer color-coded by age: green <1h, yellow <1d, orange <3d, red pulsing >3d
 *   - Escalation chain renders as avatar circles with arrow connectors; last avatar highlighted red
 *   - Validation CTA buttons ("Looks Good" / "Not Resolved") appear only when status is PENDING_VALIDATION
 *   - Material request badge, tenant-raised border, and property label are all opt-in visual hints
 */
'use client';

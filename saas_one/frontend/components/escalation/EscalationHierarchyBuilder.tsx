/**
 * EscalationHierarchyBuilder — Visual Drag-and-Drop Escalation Chain Editor
 *
 * An interactive visual builder for creating and managing multi-level escalation chains.
 * Each hierarchy consists of named levels containing one assigned employee, an escalation
 * time threshold, and a set of notification channels (push, email). Users drag employees
 * from a searchable pool into level slots, configure timing and channels per level, and
 * save the hierarchy to the backend.
 *
 * KEY PROPS:
 *   - organizationId:  string — required; the org context for fetching/saving hierarchies
 *   - propertyId?:     string — optional; scopes the hierarchy to a specific property
 *   - propertySelector?: React.ReactNode — optional node rendered at the top of the component
 *
 * KEY STATE (main component):
 *   - hierarchies:     Hierarchy[] — all escalation hierarchies for the current org/property
 *   - employees:       Employee[] — all available employees for the drag-and-drop pool
 *   - showBuilder:     boolean — toggles between list view and the editor
 *   - editingHierarchy: Hierarchy | null — when non-null, the builder is in edit mode pre-populated
 *   - hierarchyName / hierarchyDesc / triggerAfterMinutes / isDefault — form fields for the builder
 *   - levels:          EscalationLevel[] — the levels being edited in the builder
 *   - poolSearch:      string — live filter string for the employee pool
 *   - dragOverLevel:   number | null — tracks which level slot is the current drag target
 *
 * KEY SUB-COMPONENTS:
 *   - EmployeeCard: Draggable card representing an employee in the pool or in a level slot
 *   - LevelCard:   A single escalation level with drop zone, time input, and channel toggles
 *   - HierarchyCard: Collapsible summary card in the list view showing all levels as a chain
 *
 * INTEGRATION:
 *   - Fetches employees and hierarchies from internal API endpoints on mount and on property change
 *   - Builds POST/PUT bodies matching the EscalationChain table schema
 *   - Uses HTML5 drag-and-drop (native, not a library) for moving employees between levels
 *   - Auto-scrolls the level row when dragging an employee near the edges
 *   - Optimistic UI: updates local state immediately on save; reverts on error
 *
 * UI/UX PATTERNS:
 *   - Three-panel layout: employee pool (left), level slots (center), list view (when not building)
 *   - Level cards have color-coded gradient headers (blue -> amber -> orange -> red -> rose -> purple)
 *   - Drop zones highlight with a primary-colored ring when an employee is dragged over them
 *   - Time input uses a local string state that commits to number state on blur (allows clearing)
 *   - Notification channels rendered as toggle buttons (filled when active)
 *   - HierarchyCard expands inline to show all levels as a compact horizontal chain
 *   - Framer-motion layout animations for smooth reordering of hierarchy cards
 */
'use client';

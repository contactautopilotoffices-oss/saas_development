 ...
 *
 * Purpose: Sends notifications across three channels whenever a significant event occurs
 * in the system (ticket created, assigned, completed, room booked, etc.).
 *
 * Three notification channels:
 *   1. In-App Notifications (DB)  — Stored in the `notifications` table, shown in the app UI bell icon.
 *      Why: Persistent, browsable notification history.
 *   2. Web Push Notifications     — Delivered via Firebase Cloud Messaging (FCM) to browser/mobile.
 *      Why: Works even when the user has closed the tab or left the property.
 *   3. WhatsApp Messages         — Sent via WasenderAPI through the message queue.
 *      Why: Most reliable for staff on-the-go who may not have the app open.
 *
 * Recipient Logic:
 *   - Internal tickets: Tenants are excluded from notifications entirely.
 *   - Tenant notifications: A tenant only receives notifications about tickets THEY raised.
 *   - Staff/Admin/MST: Receive all notifications for their property.
 *   - org_super_admin: Receives notifications for all tickets within their org.
 *
 * Core send() function flow:
 *   1. Insert notification record into the `notifications` table (in-app).
 *   2. Fetch active push tokens for the user from `push_tokens` table.
 *   3. Dispatch push notification to each unique browser via FCM.
 *   4. Enqueue WhatsApp message via WhatsAppQueueService.
 *   5. Stale/unregistered FCM tokens are automatically deactivated.
 *
 * Trigger events (called from API route handlers):
 *   - afterTicketCreated          — New ticket raised (assigned or unassigned)
 *   - afterTicketPhotoUploaded    — Media attached to a ticket
 *   - afterTicketWaitlisted      — Ticket added to waitlist
 *   - afterTicketReassigned       — Manual reassignment by admin
 *   - afterTicketAssigned         — Ticket assigned (auto or manual)
 *   - afterTicketCompleted        — MST marks ticket as completed
 *   - afterTicketPendingValidation — Tenant needs to approve completed work
 *   - afterTicketValidated        — Tenant approves or rejects completed work
 *   - afterCriticalTicketCreated  — Critical priority ticket (urgent alert)
 *   - afterRoomBooked            — Meeting room reservation made
 *   - afterSOPItemRated          — Admin rates a staff member's SOP checklist item
 *
 * Design: All methods are static. Failures are caught and logged — never throw.
 * Notification failures must not affect the main business operation.
 */
...
/** Payload shape for a single notification dispatch */
export interface NotificationPayload {
    ...
    type: string              // Notification type constant, e.g., 'TICKET_ASSIGNED', 'ROOM_BOOKED'
    title: string            // Short notification title (shown in bell, push, WhatsApp)
    message: string          // Body text of the notification
    deepLink: string         // App URL path to navigate to when clicked
    /** Pre-built WhatsApp payload — avoids re-fetching ticket data inside send() */
    whatsapp?: {
        message: string
        mediaUrl?: string
...
/** Simple userId + role tuple returned by recipient queries */
...
export class NotificationService {
    ...
    /**
     * Triggered when a new ticket is created (with or without auto-assignment).
     *
     * Flow A — Ticket auto-assigned:
     *   1. Notify the assignee directly with "New Ticket Created & Assigned to You".
     *   2. Notify all other relevant recipients with "New Ticket Created & Assigned to [Name]".
     *
     * Flow B — Ticket unassigned (waitlisted):
     *   1. Notify all relevant recipients with "New Ticket Created" (no assignee yet).
     *
     * Recipient filtering:
     *   - Tenants: Only notified about tickets THEY created.
     *   - Staff/Admin/MST: Notified about all tickets.
     *   - org_super_admin: Added to recipient list for visibility.
     *
     * Media handling: Polls the DB up to 3 times (3s apart) because ticket creation
     * may trigger a background media upload — the photo may not be available immediately.
     */
    ...
    /**
     * Triggered when a before/after photo or video is uploaded to a ticket.
     * Sends WhatsApp messages to all recipients with the photo/video attached.
     * Why WhatsApp-only: Push/DB notification was already sent at creation time;
     * this is a follow-up with visual proof that doesn't need a DB write.
     */

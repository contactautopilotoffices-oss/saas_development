/**
 * @fileoverview SLA Breach Checker Cron API
 * @route GET /api/cron/check-sla
 * @description
 *   Scans all in-progress tickets with an assigned technician (MST) and detects
 *   those approaching their SLA deadline (within 30 minutes of the 4-hour SLA window).
 *   Inserts `sla_warning` notifications for each at-risk ticket to alert the assigned
 *   technician. Deduplicates alerts within a 25-minute window to avoid spamming.
 *
 *   Intended to be called on a schedule (e.g., every minute) by an external cron
 *   service such as Vercel Cron or GitHub Actions. Protected by a bearer token
 *   matching the `CRON_SECRET` environment variable.
 * @requires_auth Bearer token (CRON_SECRET)
 * @response 200 { success, checked, notifications_sent }

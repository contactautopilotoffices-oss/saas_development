/**
 * SLA escalation check cron job.
 * Checks ticket SLA deadlines and triggers escalation workflows.
 ...
 *
 * Checks all non-resolved tickets that belong to an escalation hierarchy and have
 * escalation not paused. For each ticket, computes the elapsed time since the last
 * escalation action (or creation date if level 0). If the elapsed time exceeds the
 * current level's timeout, the ticket is escalated to the next level and the
 * assigned employee is notified. Reaching the final level pauses escalation on that
 * ticket so it is never re-checked.
 *
 * Performance: fetches all tickets, hierarchies, and levels in 3 flat queries then
 * joins in memory — O(1) DB calls regardless of ticket count.
 *

/**
 * AMC expiry alert cron job.
 * Sends notifications for expiring AMC contracts.
 * POST /api/cron/amc-expiry-alerts
 * Auth: Cron secret.
 */
 ...
 *
 * Runs on a Vercel Cron schedule. For each expiry window (30, 15, and 7 days out),
 * queries all non-expired/non-renewed AMC contracts whose contract_end_date matches
 * the target date. Recipients include org admins and property admins for the affected
 * contract's organization and property.
 *
 * Auth: Bearer token matching CRON_SECRET environment variable (optional — allows
 * unauthenticated calls if CRON_SECRET is not set).
 */

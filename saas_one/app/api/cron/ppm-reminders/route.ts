/**
 * PPM reminder cron job.
 * Sends reminders for upcoming PPM scheduled tasks.
 * POST /api/cron/ppm-reminders
 * Auth: Cron secret.
 */
 ...
 *
 * Sends WhatsApp reminders for PPM tasks due in 3 days.
 *
 * Runs daily at 9:00 AM IST (03:30 UTC) via Vercel Cron. Finds all PPM schedule
 * records with planned_date exactly 3 days from today and status = pending, then
 * sends a formatted WhatsApp reminder to all org super admins and affected property
 * admins so vendors and staff can be pre-informed.
 *

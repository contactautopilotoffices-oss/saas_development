/**
 * Ticket listing and creation API.
 *
 * HTTP Methods: GET (list tickets with filters), POST (create a new ticket with auto-classification)
 *
 * GET — Retrieves a paginated list of tickets with optional filters.
 *   Query params:
 *     propertyId, organizationId, status, isInternal, assignedTo, raisedBy, raisedByRole,
 *     limit, offset, slaBreached, dateFrom, dateTo, search, skillGroup
 *   Returns: { tickets: Ticket[], total: number }
 *
 * POST — Creates a new ticket, auto-classifies it using a two-pass AI pipeline
 *   (rule-based pre-pass + Groq LLM), enriches it with location/floor data, and
 *   attaches the default escalation hierarchy.
 *   Body: { description, title?, propertyId, organizationId, isInternal?, priority?,
 *           department?, assignedTo? }
 *   Returns: { success: true, ticket: Ticket, classification: ClassificationResult }
 *
 * Depends on:
 *   - Supabase auth (cookie-based for web, Bearer token for mobile)
 *   - Groq LLM API (via resolveClassification / classifyTicketEnhanced)
 *   - NotificationService (fires afterTicketCreated, afterCriticalTicketCreated)
 *   - Audit logging (logClassification)
 *   - Supabase escalation_hierarchies table (auto-attaches default hierarchy)
 */

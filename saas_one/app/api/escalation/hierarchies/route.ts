/**
 * GET /api/escalation/hierarchies?organizationId=&propertyId=
 * List escalation hierarchies for an org or property. Uses adminClient for nested employee join
 * (users RLS blocks joins otherwise). Returns hierarchies with nested escalation_levels sorted by level_number.
 *
 * POST /api/escalation/hierarchies
 * Create a new escalation hierarchy with its levels. Auto-unsets is_default on other hierarchies in scope.
 * Body: { organizationId, propertyId?, name, description?, levels: [{level_number, employee_id, escalation_time_minutes, notification_channels}], trigger_after_minutes?, is_default? }
 * Validates: employee_ids exist, max 10 levels per hierarchy. Rolls back hierarchy on level insert failure.
 */

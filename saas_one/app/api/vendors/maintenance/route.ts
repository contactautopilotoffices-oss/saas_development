/**
 * @fileoverview Maintenance Vendor Management API
 * @route GET  /api/vendors/maintenance — List vendors for an org/property with PPM task counts
 * @route POST /api/vendors/maintenance — Create a vendor, auth account, and send welcome message
 * @description
 *   Provides CRUD operations for maintenance vendors within an organization.
 *   GET returns vendors scoped to a property (via `vendor_property_assignments`) or
 *   the full organization, enriched with PPM schedule task counts (total/pending/done).
 *   POST creates a vendor profile, a linked Supabase auth user (if email provided),
 *   adds them to the organization with role `maintenance_vendor`, assigns them to
 *   properties, auto-links existing PPM schedules by company name, and enqueues a
 *   WhatsApp welcome message.
 * @requires_auth Yes (session required for all operations)
 * @response 200 { vendors } — GET with optional `user_id` filter returns { vendor }

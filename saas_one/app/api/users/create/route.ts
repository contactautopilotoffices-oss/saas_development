/**
 * @fileoverview User Creation API (Admin-Initiated)
 * @route POST /api/users/create
 * @description
 *   Creates a new user account directly (no email verification required) and
 *   provisions the appropriate organization/property memberships based on the
 *   requested role. Handles special cases:
 *
 *   - `org_super_admin` / `procurement` roles → added to `organization_memberships`
 *   - `staff` / `mst` roles → added to `property_memberships`, with optional
 *     `mst_skills` and `resolver_stats` entries
 *   - `master_admin` creation → restricted to existing master admins only;
 *     sets `is_master_admin = true` on the user record
 *
 *   On any membership insertion failure after the auth user is created, the
 *   function rolls back by deleting the auth user to avoid stranded accounts.
 *   A temporary WhatsApp welcome message is sent asynchronously (non-blocking)
 *   if a phone number is provided.
 *
 *   Security: Only accepts explicitly allowlisted role values to prevent
 *   privilege escalation. Never returns passwords in the API response.
 * @requires_auth Yes — org admin, property admin, or master admin
 * @request_body { email, full_name, organization_id, role?, password?, phone?,
 *                  property_id?, specialization?, skills?, create_master_admin? }
 * @response 200 { success, user, requiresPasswordReset }
 * @response 400 Bad Request — missing/invalid fields or disallowed role
 * @response 401 Unauthorized
 * @response 403 Forbidden — caller lacks permission to create this role

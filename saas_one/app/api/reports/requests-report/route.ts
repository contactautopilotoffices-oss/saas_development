/**
 * Ticket analytics and monthly requests/snags report for a property.
 *
 * HTTP Methods: GET
 *
 * GET — Returns KPI metrics, floor-based and department-based breakdown charts,
 *   and detailed ticket records for a property within a given date range.
 *   Query params:
 *     propertyId (required): UUID of the property
 *     month: YYYY-MM format (mutually exclusive with startDate+endDate)
 *     startDate, endDate: YYYY-MM-DD custom date range
 *   Returns:
 *     { success: true, month: { value, label }, property: { name, code, address },
 *       kpis: { totalSnags, closedSnags, openSnags, pendingValidationCount,
 *               isValidationEnabled, closureRate },
 *       charts: { floor: { labels, data }, department: { labels, open, closed } },
 *       floorGroups: Record<string, Ticket[]>, tickets: FormattedTicket[] }
 *
 * Depends on:
 *   - Supabase auth (session required)
 *   - Supabase tickets, properties, property_features, ticket_activity_log tables
 *   - ticket_validation feature flag (is_enabled defaults to true if absent)
 */

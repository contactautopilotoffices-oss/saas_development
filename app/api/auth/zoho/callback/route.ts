    ...
    try {
        ...
        const tokenRes = await fetch('https://accounts.zoho.com/oauth/v2/token', {
            ...
            body: new URLSearchParams({
                ...
                redirect_uri: `${requestUrl.origin}/api/auth/zoho/callback`,
        ...
        if (dbUser?.is_master_admin || user.user_metadata?.is_master_admin) {
            return NextResponse.redirect(`${requestUrl.origin}${redirect || '/master'}`);
        }
        ...
        if (orgMembership) {
            return NextResponse.redirect(
                `${requestUrl.origin}${redirect || `/org/${orgMembership.organization_id}/dashboard`}`
            );
        }
        ...
        if (propMembership) {
            ...
            return NextResponse.redirect(
                `${requestUrl.origin}${redirect || roleRoutes[role] || `/property/${pId}/dashboard`}`
            );
        }
